class CreateDuplicateRegByOriginView < ActiveRecord::Migration
    def up
        ...
        CREATE MATERIALIZED VIEW duplicate_reg_by_origin AS
          WITH registrations AS (
            ...
            WHERE action = 'approve' AND
                  notes  IN ('acceptDuplicate', 'cancelDuplicate') AND
    ...
    def down
      self.connection.execute %Q{
        DROP MATERIALIZED VIEW IF EXISTS duplicate_reg_by_origin ;
      }
    end
end
