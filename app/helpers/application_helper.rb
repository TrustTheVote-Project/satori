module ApplicationHelper
  ...
  def section_row(title, section_id, options = nil)
    ...
    content = [
      content_tag(:div, content_tag(:h4, title), class: 'col-xs-10')
    ]

    if state != :always_open
      content << content_tag(:div, content_tag(:button, state == :opened ? hide_label : show_label, :class => 'btn btn-xs btn-default', :type => 'button', :data => { show_label: show_label, hide_label: hide_label, toggle: 'collapse', target: "##{section_id}" }, "aria-expanded" => false, "aria-controls" => section_id), class: 'col-xs-2 text-right')
    end
